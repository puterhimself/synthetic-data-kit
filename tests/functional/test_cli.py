...
@patch("synthetic_data_kit.utils.directory_processor.process_directory_save_as")
@patch("synthetic_data_kit.core.save_as.push_to_huggingface_hub")
def test_save_as_directory_hub_path_appends_basename(
    mock_push, mock_process_directory, patch_config, tmp_path
):
    """Ensure --hub-path without trailing slash appends file names for multiple outputs."""

    runner = CliRunner()

    output_dir = tmp_path / "final"
    output_dir.mkdir()
    file_one = output_dir / "conv_one.jsonl"
    file_two = output_dir / "conv_two.jsonl"
    file_one.touch()
    file_two.touch()
    ...
    mock_process_directory.return_value = {
        ...
        "results": [
            {
                "input_file": str(tmp_path / "curated1.json"),
                "output_file": str(file_one),
                "format": "conversation",
                "storage": "json",
                "status": "success",
            },
            {
                "input_file": str(tmp_path / "curated2.json"),
                "output_file": str(file_two),
                "format": "conversation",
                "storage": "json",
                "status": "success",
            },
    ...
    mock_push.side_effect = [
        "https://huggingface.co/datasets/tester/dataset",
        "https://huggingface.co/datasets/tester/dataset",
    ]

    with patch.dict(os.environ, {"HUGGINGFACE_TOKEN": "hf_token_multi"}, clear=False):
        result = runner.invoke(
            app,
            [
                "save-as",
    ...
    paths = [kwargs["path_in_repo"] for _, kwargs in mock_push.call_args_list]
    assert paths == [
        "multi/conversations/conv_one.jsonl",
        "multi/conversations/conv_two.jsonl",
    ]
...
@patch("synthetic_data_kit.utils.directory_processor.process_directory_save_as")
@patch("synthetic_data_kit.core.save_as.push_to_huggingface_hub")
def test_save_as_directory_hub_path_respects_single_file_name(
    mock_push, mock_process_directory, patch_config, tmp_path
):
    """Single file uploads honor explicit filenames in --hub-path."""

    runner = CliRunner()

    output_dir = tmp_path / "final"
    output_dir.mkdir()
    output_file = output_dir / "conv_one.jsonl"
    output_file.touch()
    ...
    with patch.dict(os.environ, {"HUGGINGFACE_TOKEN": "hf_token_single"}, clear=False):
        result = runner.invoke(
            app,
            [
                "save-as",
    ...
    assert result.exit_code == 0
    mock_push.assert_called_once()
    assert mock_push.call_args[1]["path_in_repo"] == "custom_name.jsonl"
