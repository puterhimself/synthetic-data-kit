...
def aggregate_insights(file_analyses: List[FileAnalysis]) -> Dict[str, Any]:
    ...
    aggregate['categories'] = dict(sorted(category_counts.items(), key=lambda x: x[1], reverse=True))
...
def analyze_path(
    ...
    stats = {
        ...
        'avg_extraction_time_ms': sum(f.extraction_time_ms for f in file_analyses) / len(file_analyses) if file_analyses else 0,
        'avg_analysis_time_ms': sum(f.analysis_time_ms for f in file_analyses) / len(file_analyses) if file_analyses else 0,
    }
