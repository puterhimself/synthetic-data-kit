...
def process_file(
    ...
    if content_type == "qa":
        ...
        # Get num_pairs from plan, args, or config
        if plan and file_plan:
            num_pairs = file_plan.num_qa
            # Set prompt family if specified
            if file_plan.prompt_family:
                generator.prompt_family = file_plan.prompt_family
        elif num_pairs is None:
            config = client.config
            generation_config = get_generation_config(config)
            num_pairs = generation_config.get("num_pairs", 25)

        # Dry run: just return planned counts
        if dry_run:
            print(f"[DRY RUN] Would generate {num_pairs} QA pairs for {file_path}")
            return None

        # Process document
        result = generator.process_documents(
            documents, num_pairs=num_pairs, verbose=verbose, rolling_summary=rolling_summary
        )
        ...
        output_path = generator.process_dataset(
            documents=documents, output_dir=output_dir, num_examples=num_pairs, verbose=verbose
        )
        return output_path
