...
def process_directory_ingest(
    ...
    if not supported_files:
        ...
        return {"total_files": 0, "successful": 0, "failed": 0, "results": [], "errors": []}
        ...
        for file_path in supported_files:
            ...
            try:
                ...
                output_path = process_file(
                    file_path, output_dir, None, config, multimodal=multimodal
                )

                # Record success
                results["successful"] += 1
                results["results"].append(
                    {"input_file": file_path, "output_file": output_path, "status": "success"}
                )

                if verbose:
                    console.print(
                        f"✓ Processed {filename} -> {os.path.basename(output_path)}", style="green"
                    )
                else:
                ...
                results["errors"].append(
                    {"input_file": file_path, "error": str(e), "status": "failed"}
                )
...
def process_directory_create(
    ...
    if plan and plan.file_plans:
        plan_lookup = {
            str(Path(file_plan.file_path).resolve()): file_plan for file_plan in plan.file_plans
        }
    ...
    with Progress(
        ...
        for file_path in supported_files:
            ...
            try:
                ...
                if verbose:
                    console.print(
                        f"✓ Curated {filename} -> {os.path.basename(result_path)}", style="green"
                    )
                else:
                    console.print(f"✓ {filename}", style="green")
...
def process_directory_save_as(
    ...
    console.print(
        f"Found {len(supported_files)} JSON files to convert to {format} format", style="blue"
    )
        ...
        for file_path in supported_files:
            ...
            try:
                ...
                result_path = convert_format(
                    file_path, output_path, format, config, storage_format=storage_format
                )
                ...
                if verbose:
                    console.print(
                        f"✓ Converted {filename} -> {os.path.basename(result_path)} ({format}, {storage_format})",
                        style="green",
                    )
                else:
                    console.print(f"✓ {filename}", style="green")
